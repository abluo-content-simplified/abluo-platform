...
4. **If the lock exists:** determine whether a genuine **git** process currently owns it — check file-scoped, e.g. `lsof .git/index.lock`, and inspect the owning PID's full command (`ps -p <pid> -o comm=`). **Never delete the lock merely because it exists. Never kill processes found via broad pgrep/pkill patterns.** Known false positive (verified 2026-07-23): the Cowork sandbox VM (`com.apple.Virtualization.VirtualMachine`) holds a *read-only* handle on locks its own git created but could not unlink — that is not ownership; a lock is genuinely owned only when a git process holds it. Guarded cleanup command (removes unless a git process holds it): `lsof .git/index.lock 2>/dev/null | grep -qi git && echo "LOCK OWNED BY GIT — do not remove" || rm -f .git/index.lock`.
