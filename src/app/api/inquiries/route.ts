...
export async function POST(request: Request) {
  try {
    ...
    return await runAsTrustedSystemOperation(
      'Anonymous public inquiry POST (early-access / contact form) — no user session exists ' +
        '(the submitter is a website visitor, never an authenticated tenant/project member). ' +
        'Covers: the isRateLimited() per-IP count query, the tenants/projects slug lookups, ' +
        'and the inquiries INSERT itself.',
      async (supabase) => {
        // ── Spam protection ──────────────────────────────────────────────────
