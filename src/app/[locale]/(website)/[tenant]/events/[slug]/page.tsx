...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const event = await fetchForTenant<Event>(eventBySlugQuery, { slug, locale, defaultLocale })
...
export async function generateStaticParams() {
  // This is optional — can be empty for dynamic routes
  return []
}
...
export default async function EventDetailPage({ params }: PageProps) {
  ...
  // Get related events (other events, up to 3)
                ...
                {relatedEvents.map((relEvent: Event) => {
                  ...
                  return (
                    <Link
                      ...
                      href={`/${locale}/livener/events/${relEvent.slug.current}`}
