...
//
// Defense-in-depth alongside the live-DB suite
// (supabase/verify/live-rls.verify.mjs, describe block "(e) inquiries
// authz — migration 014", which is the suite that actually proves the SQL
// GRANT + RLS policies enforce isolation against a real Postgres). This
// block is I/O-free — it mocks `@/lib/supabase/server` and `@/lib/api/auth`
// — so it can only prove the ROUTE's own logic fails closed correctly given
// each possible Supabase/auth response shape; it cannot and does not prove
// the RLS policies themselves are correct (that is exactly what the live-DB
// suite is for). Both are required; neither substitutes for the other.
describe('ADR-017 slice 5 — PATCH /api/inquiries/[id] fails closed at every stage', () => {
  async function loadRouteWithMocks(params: {
    actor: { userId: string; platformRole: 'abluo_admin' | 'tenant_user' } | null
    ...
    vi.doMock('@/lib/api/auth', () => ({
      getAuthenticatedActor: vi.fn().mockResolvedValue(params.actor),
    }))

    const selectResult = params.selectResult ?? { data: null, error: null }
    const updateResult = params.updateResult ?? { data: null, error: null }
    ...
    const route = await import('@/app/api/inquiries/[id]/route')
    return { route, from, selectFn, updateFn }
  }

  const validId = '11111111-1111-1111-1111-111111111111'
  function makeRequest(body: Record<string, unknown> = {}) {
    return new Request(`http://localhost/api/inquiries/${validId}`, {
      method: 'PATCH',
      body: JSON.stringify(body),
    })
  }

  it('unauthenticated caller (no session) is rejected with 401 BEFORE any Supabase call — anonymous fails closed', async () => {
    const { route, from } = await loadRouteWithMocks({ actor: null })

    const res = await route.PATCH(makeRequest(), { params: Promise.resolve({ id: validId }) })

    expect(res.status).toBe(401)
    expect(from).not.toHaveBeenCalled()
  })

  it('an authenticated but cross-tenant caller (RLS filters the SELECT to 0 rows) gets 404 — never a 403 that would confirm the row exists for another tenant', async () => {
    const { route } = await loadRouteWithMocks({
      actor: { userId: 'user-cross-tenant', platformRole: 'tenant_user' },
      selectResult: { data: null, error: null }, // RLS-filtered: 0 rows, not an error
    })
  ...
  it('a viewer (can SELECT, but the UPDATE policy denies the write — 0 rows returned) gets 403, distinct from the 404 case', async () => {
    const { route } = await loadRouteWithMocks({
      actor: { userId: 'user-viewer', platformRole: 'tenant_user' },
      selectResult: { data: { id: validId, data: {}, email: 'x@x.test' }, error: null },
      updateResult: { data: [], error: null }, // UPDATE policy denied — 0 rows, not an error
    })

    const res = await route.PATCH(makeRequest({ organization: 'Acme' }), {
  ...
  it('a legitimate owner/editor (SELECT visible, UPDATE affects 1 row) succeeds — positive control', async () => {
    const { route, from } = await loadRouteWithMocks({
      actor: { userId: 'user-owner', platformRole: 'tenant_user' },
      selectResult: { data: { id: validId, data: {}, email: 'x@x.test' }, error: null },
      updateResult: { data: [{ id: validId }], error: null },
    })
    ...
    // Confirms the route used the RLS-scoped session client's `.from()` —
    // i.e. never fell back to an admin/service-role client for this route.
