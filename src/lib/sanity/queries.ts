...
// ─── Tenant-scoped formDefinition id lookup ──────────────────────────────────
//
// `scopedFormDefinition` for surfaces that need ONLY the route key (CTA_FIELDS).
// Identical filter — deliberately, so `__tests__/query-tenant-scope.test.ts`
// sees one shape for every reference-keyed form subquery — projecting the
// single `formId` field instead of the whole definition.
const scopedFormId = (field: string) => /* groq */ `*[
        _type == "formDefinition"
        && _id == ^.${field}._ref
        && (role == "template" || tenantSlug == ${PROJECT_TENANT_SLUG})
      ][0].formId`
...
export const CTA_FIELDS = /* groq */ `
  ...
  // TENANT-SCOPED — the same filtered subquery as scopedFormDefinition(), one
  // field wide. A bare formRef-> read the route key off ANY document by id, so
  // a CTA pointing (by a copied document id, or a picker with no tenant clause)
  // at another client's form yielded that client's formId — and the overlay
  // opens BY formId. Narrower than rendering a foreign definition, but the same
  // crossing out of the project scope the root filter established.
  //
  // This closes the leak at the READ that produces the id. It does NOT audit
  // what the overlay then does with it: resolveActiveDefinition(formId,
  // tenantSlug) in src/lib/forms/definition-source.ts and the
