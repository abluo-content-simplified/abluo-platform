...
//
// `resolveProjectScope` / `activeFormReferenceFilter` moved to
// `@/lib/sanity/form-reference-filter` so that `@/lib/modules/config-schema`
// (which generates module-config pickers) can share the SAME function object
// without importing this file — see that module's header for the cycle. They
// are re-exported here because this file is their public seam and the schema's
// tests import them from it.
export {
  resolveProjectScope,
  activeFormReferenceFilter,
} from '@/lib/sanity/form-reference-filter'
